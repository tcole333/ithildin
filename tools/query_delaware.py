#!/usr/bin/env python3
"""
Delaware corporate registry via OpenCorporates API.
...
Usage:
    python tools/query_delaware.py search "ENTITY NAME"
    python tools/query_delaware.py search "ENTITY NAME" --inactive
    python tools/query_delaware.py entity <company_number>
    python tools/query_delaware.py filings <company_number>
    python tools/query_delaware.py batch-entities <entity1> <entity2> ...
"""
...
def search_companies(query, inactive=False, per_page=30, page=1):
    """
    Search Delaware companies by name.
    ...
    try:
        log_search("delaware_opencorporates", query, {"result_count": result_count})
    except Exception:
    ...
    for item in companies:
        ...
        results.append({
            ...
            "current_status": company.get("current_status"),  # May be None for Delaware
...
def get_company(company_number):
    ...
    Args:
        company_number: Delaware company/file number
    ...
    try:
        log_search("delaware_opencorporates", f"entity:{company_number}", {"found": True})
    except Exception:
        pass
...
def get_filings(company_number, per_page=100, page=1):
    ...
    try:
        result_count = data.get("results", {}).get("total_count", 0)
        log_search("delaware_opencorporates", f"filings:{company_number}", {"result_count": result_count})
    except Exception:
        pass
...
def batch_entities(company_numbers, output_file=None):
    ...
    Args:
        company_numbers: List of Delaware company numbers
...
def main():
    parser = argparse.ArgumentParser(
        description="Delaware corporate registry via OpenCorporates API"
    )
    sub = parser.add_subparsers(dest="command", help="Command")

    # Search
    search_p = sub.add_parser("search", help="Search Delaware companies by name")
    ...
    entity_p.add_argument("company_number", help="Delaware company/file number")
    add_output_args(entity_p)

    # Filings
    filings_p = sub.add_parser("filings", help="Get company filings")
    filings_p.add_argument("company_number", help="Delaware company/file number")
