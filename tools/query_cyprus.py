#!/usr/bin/env python3
"""
Cyprus corporate registry via OpenCorporates API.
...
Key targets: Xitrans Finance Ltd (Rybolovlev-Epstein), Deripaska entities,
Russian oligarch offshore structures. Cyprus is major Russia-linked offshore hub.

Usage:
    python tools/query_cyprus.py search "ENTITY NAME"
    python tools/query_cyprus.py search "ENTITY NAME" --inactive
    python tools/query_cyprus.py entity <company_number>
    python tools/query_cyprus.py filings <company_number>
    python tools/query_cyprus.py batch-entities <entity1> <entity2> ...
"""
...
def search_companies(query, inactive=False, per_page=30, page=1):
    """
    Search Cyprus companies by name.
    ...
    try:
        log_search("cyprus_opencorporates", query, {"result_count": result_count})
    except Exception:
...
def get_company(company_number):
    ...
    Args:
        company_number: Cyprus company registration number
    ...
    try:
        log_search("cyprus_opencorporates", f"entity:{company_number}", {"found": True})
    except Exception:
        pass
...
def get_filings(company_number, per_page=100, page=1):
    ...
    try:
        result_count = data.get("results", {}).get("total_count", 0)
        log_search("cyprus_opencorporates", f"filings:{company_number}", {"result_count": result_count})
    except Exception:
        pass
...
def batch_entities(company_numbers, output_file=None):
    ...
    Args:
        company_numbers: List of Cyprus company numbers
...
def main():
    parser = argparse.ArgumentParser(
        description="Cyprus corporate registry via OpenCorporates API"
    )
    sub = parser.add_subparsers(dest="command", help="Command")

    # Search
    search_p = sub.add_parser("search", help="Search Cyprus companies by name")
    ...
    entity_p.add_argument("company_number", help="Cyprus company registration number")
    add_output_args(entity_p)

    # Filings
    filings_p = sub.add_parser("filings", help="Get company filings")
    filings_p.add_argument("company_number", help="Cyprus company registration number")
