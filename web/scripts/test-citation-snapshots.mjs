...
if (current.dossier_darren_indyke.has_curation_prose) {
  assert.ok(current.dossier_darren_indyke.support_span_line, "Expected Darren support span sample to be present when curation prose exists.");
  if (current.dossier_darren_indyke.has_footnotes) {
    assert.ok(current.dossier_darren_indyke.finding_footnote, "Expected Darren finding footnote to be present when footnotes are rendered.");
