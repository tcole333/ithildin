...
export async function processArticleEvidenceContent(
  ...
  const renderedHtml = await marked.parse(contentWithCitations);
...
export function processDossierCurationEvidence(input: DossierCurationInput): ProcessDossierResult {
  ...
  const leadHtml = rawLead
    ? applyCitations(rawLead, { findingEvidenceMap }, citationState).markdown
    : "";

  const processedSections = rawSections.map((section) => {
    const content = typeof section?.content === "string" ? section.content : "";
    const processedContent = content
      ? applyCitations(content, { findingEvidenceMap }, citationState).markdown
      : "";
    return {
      ...section,
      processedContent,
  ...
  const legacyOverviewHtml = rawLegacyOverview
    ? applyCitations(rawLegacyOverview, { findingEvidenceMap }, citationState).markdown
    : "";

  const legacyFinancialSummaryHtml = rawLegacyFinancialSummary
    ? applyCitations(rawLegacyFinancialSummary, { findingEvidenceMap }, citationState).markdown
    : "";
