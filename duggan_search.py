...
def main():
    ...
    if args.json:
        output = json.dumps({"query": args.query, "total": total, "hits": hits}, indent=2)
        if args.output:
            with open(args.output, "w") as f:
                f.write(output)
            print(f"Saved {len(hits)} results to {args.output}")
        else:
            print(output)
    else:
    ...
    if args.output and not args.json:
        with open(args.output, "w") as f:
            json.dump({"query": args.query, "total": total, "hits": hits}, f, indent=2)
        print(f"\nSaved {len(hits)} results to {args.output}")
