...
| DS10 financial export | Filtered DS10 flow graph for UI | `content/financials/ds10-flows.json` | 67 nodes, 63 links, 50 top transactions, 4 balance entities / 78 points | top tx sender: `35/50` (70.0%); receiver: `36/50` (72.0%); reference: `21/50` (42.0%); EFTA ID: `50/50`; export filter retains `304/724` rows after counterparty/self/internal filters and `73/724` rows (10.1%) after `>= $50k` threshold | `Medium` |
| Apollo flow model | Curated flow model + linked findings | `content/financials/apollo-pipeline.json` | 8 nodes, 7 links, 114 findings | dated findings: `110/114` (96.5%); confidence populated: `114/114`; link values are curated constants, not exhaustive transaction ledger | `Medium` |
| Wexner architecture model | Curated structure/trust model | `content/financials/wexner-architecture.json` | 8 nodes, 7 links | labeled links: `7/7`; amount links: `3/7`; structural unit-value links (`value=1`): `4/7` | `Low-Medium` (conceptual scaffold) |
...
4. Add DS10 canonicalization/dedupe before `pipeline/export_financials.py`.
