# Investigative Methodology for LLM Agents

This document defines how to think as an investigator, not just how to run queries. The tools are the hands; this is the brain.
...
### 1. Think Predictively, Then Verify

LLMs are extraordinary predictors and simulators of human behavior. Use this.
...
3. **Search systematically** across all sources, logging each search.
...
An entity discovered through a phone pivot, address pivot, or corporate filing that has zero email corpus results is NOT a "false positive." It may be:
...
1. **Verify the pivot** — confirm the linking data point (which phone number? which address? which filing?)
2. **Investigate the entity on its own terms** — corporate filings, website, officers, address, business type
3. **Search the corpus** — but treat zero results as informational, not dispositive
4. **Map the people** — who are the officers/principals? Do THEY appear in the corpus even if the entity doesn't?
...
**You are not just an analyst — you are an intelligence infrastructure builder.** Every investigation should leave the platform better than you found it. When you encounter a gap in coverage, don't just note it — fix it or create the lead to fix it.

### Identify Source Gaps During Investigation

As you investigate, constantly ask:
- **What database SHOULD exist for this?** If you're researching a UK entity and there's no UK Companies House tool, that's an infrastructure gap. If you find a Swiss bank account and there's no Swiss commercial registry tool, flag it.
- **What jurisdiction am I missing?** Every entity is registered somewhere. If that jurisdiction isn't in our registry, it's a gap worth filling.
- **What public API could answer this?** Government databases, regulatory filings, corporate registries — most are free and have APIs or bulk downloads. If you find yourself doing manual web lookups for something that should be automated, that's a tool waiting to be built.
- **What format is the data in?** PDFs, CSVs, SFTP bulk downloads, REST APIs, SODA endpoints — each has a known integration pattern. If you find a valuable data source during research, note its format and access method.

### When You Find a Gap, Act

**Tier 1 — Build it now** (if the source is clearly accessible and would immediately help the current investigation):
- Probe the source (fetch the page, test the API, check authentication requirements)
- If it works: build the tool, test it, update CLAUDE.md and /search-all-sources
- If it needs auth or has CAPTCHAs: create a human_action item with specific steps

**Tier 2 — Create an infrastructure lead** (if it would help future investigations):
```bash
.venv/bin/python3 tools/lead_tracker.py add \
  --title "Build [SOURCE] integration tool" \
  --category infrastructure \
  --priority medium \
  --target "[SOURCE_NAME]" \
  --description "Found during [TARGET] investigation. [SOURCE] at [URL] has [WHAT_DATA]. Access method: [API/bulk/scrape]. Auth: [none/key/paid]. Would help with: [USE_CASE]."
```

**Tier 3 — Note it in findings** (if it's a one-off observation):
- Record as a finding with type "infrastructure" noting the gap and potential source

### What Proactive Agents Look For

During every investigation, scan for:
- **Foreign corporate registries** mentioned in documents (Cayman, BVI, Liechtenstein, Luxembourg, Jersey, Guernsey, Isle of Man, Cyprus, Panama, Switzerland)
- **Court dockets** referenced in emails or filings (SDNY, EDNY, state courts, foreign proceedings)
- **Financial databases** referenced in filings (FinCEN, specific bank records, wire transfer systems)
- **Government publications** (Congressional hearing transcripts, IG reports, GAO audits, foreign parliamentary inquiries)
- **Specialized industry databases** (shipping registries, mining permits, real estate tax rolls, import/export records)
- **News archives** beyond GDELT's 3-month window (newspaper morgues, LexisNexis, specific publication archives)
- **Academic/research databases** (SSRN, university affiliations, grant databases, patent filings)

### Extend Existing Tools

Sometimes the gap isn't a missing tool but a missing feature:
- A registry tool that covers FL but not the state where you just found an entity → add the state
- A search tool that doesn't handle name variants well → improve the fuzzy matching
- A query tool that returns results but doesn't auto-register entities → add entity registration
- A tool that works for one document type but not a related type → extend it

When you extend a tool, update CLAUDE.md and the relevant skill files.

### The Infrastructure Multiplier

One good tool serves every future investigation. A 30-minute tool build that saves 10 minutes per investigation across 50+ targets is a 25x return. **Prioritize infrastructure work that has the highest reuse value.** A corporate registry tool for a major offshore jurisdiction (BVI, Cayman, Liechtenstein) is more valuable than a niche tool for a single document type.
...
Leads and findings can be assigned to a thread via `--thread-id N`. Unthreaded items (thread_id=NULL) belong to the general investigation.
...
- A lead spawned from one thread clearly belongs to another

### Cross-Thread Connections

The highest-value findings often bridge threads. When investigating within Thread A and you surface something relevant to Thread B:
1. Record the finding in Thread A (where you found it)
...
When an agent claims a threaded lead, it should:
1. Check existing findings in that thread (`--thread-id N`) to avoid duplicate work
2. Assign new findings to the same thread
...
- Don't speculate about criminal conduct without documentary support
- Don't pursue conspiracy theories that require massive coordinated cover-ups when simpler explanations exist
