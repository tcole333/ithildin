...
uv run python tools/duggan_search.py "<QUERY>" -n 20 --output $WORKDIR/search-duggan.json
...
uv run python tools/query_lmsband.py search "<QUERY>" --limit 20 --output $WORKDIR/search-lmsband.json
...
uv run python tools/query_unified.py docs "<QUERY>" --limit 20 --output $WORKDIR/search-unified-docs.json
...
uv run python tools/ingest_uk_companies_house.py search "<QUERY>" --limit 10
...
# California SoS (BE Public Search API — top 150 results per keyword)
uv run python tools/ingest_california.py search "<QUERY>" --output $WORKDIR/search-ca-sos.json

# Epstein Files 20K (House Oversight docs — HOUSE_OVERSIGHT IDs, not EFTA)
uv run python tools/ingest_epstein_20k.py search "<QUERY>" --limit 10

# EpsteinExposed.com (persons, documents, flights)
uv run python tools/ingest_epstein_exposed.py search "<QUERY>"

# MuckRock FOIA (21 Epstein FOIA requests — FBI, CBP, USMS, DOJ, BOP, SDNY, SDFL)
uv run python tools/query_muckrock.py search "<QUERY>"

# DocumentCloud (6,613+ pages — Giuffre v. Maxwell, MCC records)
uv run python tools/query_documentcloud.py search "<QUERY>" --limit 10
...
log_search("<QUERY>", "ca_sos", result_count)
