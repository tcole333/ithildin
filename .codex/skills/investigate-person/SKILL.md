...
uv run python tools/query_edgar.py search "<NAME>" "epstein" --size 10 --output $WORKDIR/inv-edgar-epstein.json
...
If Neo4j is running:
```bash
uv run python tools/query_icij.py search "<NAME>"
uv run python tools/query_icij.py search "<KNOWN_ENTITY>"  # If associated companies known
```
