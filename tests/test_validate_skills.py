...
Validate current runtime metadata, unknown fields, and malformed frontmatter.
"""
...
from scripts.skill_metadata import runtime_for_path
...
class TestValidateSkillFrontmatter:
    ...
    def test_valid_skill_without_user_invocable_passes(self) -> None:
        # The .codex mirror tree omits user-invocable by convention.
        text = "---\nname: my-skill\ndescription: Does a thing\n---\n\n# body\n"
        assert validate_skill_frontmatter(text, "claude") == []

    def test_unknown_key_fails_and_is_named(self) -> None:
        text = "---\nname: my-skill\ndescription: Does a thing\nfavorite_color: blue\n---\n"
        errors = validate_skill_frontmatter(text, "claude")
        assert len(errors) == 1
        assert "favorite_color" in errors[0]
        assert "user-invocable" in errors[0]  # allowlist shown in the message
    ...
    def test_non_boolean_user_invocable_fails(self) -> None:
        ...
        errors = validate_skill_frontmatter(text, "claude")
        assert errors == ["`user-invocable` must be a boolean"]
    ...
    def test_allowlist_includes_repo_and_base_keys(self) -> None:
        assert "user-invocable" in ALLOWED_SKILL_KEYS
...
class TestRealSkillTrees:
    def test_all_repo_skills_pass(self, repo_root: Path) -> None:
        ...
        failures = {
            ...
            if (errors := validate_skill_frontmatter(skill_md.read_text(encoding="utf-8"), runtime_for_path(skill_md)))
        }
        assert failures == {}
